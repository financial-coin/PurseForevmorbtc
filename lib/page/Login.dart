...
class _MyAppState extends State<Login> {
  ...
  void createWallet(bool isMainnet) async {
    ...
    final privateKeyHex = bytesToHex(btcKey.privateKey, include0x: true);

    // final network = isMainnet ? bitcoin.bitcoin : bitcoin.testnet;
    // final wallet = bitcoin.Wallet.random(network);
    // btcAddress = wallet.address;

    // print('EVM 地址: $evmAddress');
    // print('EVM 私钥: $privateKeyHex');
    print('BTC 地址: $btcAddress');
    print('BTC 私钥: $privateKeyHex');
  ...
  void checkTransactionHistory() async {
    try {
      ...
      final evmTxs = await walletBalance.getEVMTransactions(
          '0x4569b4E52F710c84864d0536D9bd61126CF69468' // 你的 EVM 地址
          );
...
// ETC 交易
  ...
  login() async {
    ...
    final map = await WalletRestore.restoreFromMnemonic(
        "craft decrease develop original attend trust domain slow hello chief tiny country");
    print('map: $map');
  }
