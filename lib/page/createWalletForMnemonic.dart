...
class CreateWalletForMnemonic extends StatefulWidget {
  @override
  // ignore: library_private_types_in_public_api
  _CreateWalletForMnemonicPageState createState() =>
      _CreateWalletForMnemonicPageState();
}

class _CreateWalletForMnemonicPageState extends State<CreateWalletForMnemonic> {
  ...
  void createWallet(bool isMainnet) async {
    ...
    print('助记词: $mnemonic');
    List<String> wordsList = mnemonic.split(' '); // 按空格分割字符串
    setState(() {
      mnemonicWords = wordsList;
      words = wordsList;
    });
    ...
    print('助记词: $mnemonic');
    print('EVM 地址: $evmAddress1');
    print('EVM 私钥: $privateKeyEVMHex');
    print('BTC 地址: $btcAddress1');
    ...
    setState(() {
      ...
      evmPrikey = privateKeyEVMHex;
      btcAddress = btcAddress1;
      btcPrikey = privateBTCKeyHex;
      btcWIF = btcKey.toWIF();
    });
  }
  ...
  SaveMnemonic() async {
    ...
    // await prefs.setString('mnemonic', mnemonicW!);
    // await WalletStorage.updateWallet(keyName!, {
    //   'mnemonic': mnemonicW,
    // });
    Clipboard.setData(ClipboardData(text: mnemonicW!));
    Get.snackbar(
      '复制成功',
      '助记词已复制到剪贴板',
...
//生成key的方法
  ...
  savePrivateKey() async {
    // final keyName = await _getNextAccountName();

    // 保存钱包数据到 WalletStorage
    // await WalletStorage.updateWallet(keyName!, {
    ...
    networkController.updateAccount(keyName!);

    print('savePrivateKey');
    String str = "btcWIF: ${btcWIF!}\nevmPrikey: ${evmPrikey!}";
  ...
  submitMnemonic() {
    ...
    List<String> arr = mnemonicW!.split(' ');
  ...
  setPwd() async {
    if (flag && _isChecked2) {
      ...
      // 获取当前账号名称
      // final accountName = await _getNextAccountName();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: CustomAppBar(
            currentPage: _currentPage, pageCount: pageCount), // 使用自定义导航栏
      ),
      body: Column(
        children: [
          Expanded(
            child: PageView(
              ...
              children: [
                _buildPage1('', ''),
                _buildPage2('', ''),
                _buildPage4('页面 3', '内容 3'),
                _buildPage5('页面 4', '内容 4'),
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
                      ...
                      itemBuilder: (context, index) {
                        return Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            // border: Border.all(color: Colors.white), // 添加边框
                            image: const DecorationImage(
                              image: AssetImage(
                                  'lib/images/borderBG.png'), // 添加背景图片
                              fit: BoxFit.fill, // 适应容器大小
                            ),
                          ),
                          child: Column(
            ...
            // SizedBox(height: 20),
            // Text(
            //   '支持任意钱包的助记词或私钥导入，XXX Wallet 不会记录你的助记词或私钥。',
  ...
  Widget _buildPage3(String title, String content) {
    return Scaffold(
      ...
      //   title: Text('钱包创建完成', style: TextStyle(color: Colors.white)),
      //   actions: [
      //     TextButton(
      //       onPressed: () {
      //         // 跳过逻辑
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const Icon(Icons.clear_outlined, color: Colors.red, size: 20),
                const Text(
                  "请勿复制保存",
                  style: TextStyle(color: Colors.white, fontSize: 14),
                ),
            ...
            SizedBox(height: 60),
            _buildInfoRow('助记词相当于你的钱包密码', '获取助记词等于获取资产所有权，一旦遗失资产将不再安全',
                'ic_write.png', Colors.blue),
            SizedBox(height: 20),
            _buildInfoRow('助记词相当于你的钱包密码', '获取助记词等于获取资产所有权，一旦遗失资产将不再安全',
                'ic_password.png', Colors.red),
            SizedBox(height: 20),
            _buildInfoRow('助记词相当于你的钱包密码', '获取助记词等于获取资产所有权，一旦遗失资产将不再安全',
                'ic_safe.png', Colors.green),
            SizedBox(height: 60),
            Container(
                padding: EdgeInsets.all(8.0),
                margin: EdgeInsets.all(8.0),
                decoration: BoxDecoration(
                  color: Color.fromRGBO(255, 224, 178, 0.3), // 背景颜色
