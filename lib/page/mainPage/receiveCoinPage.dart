...
class ReceiveCoinPage extends StatefulWidget {
  const ReceiveCoinPage({super.key});

  @override
  State<ReceiveCoinPage> createState() => _ReceiveCoinPageState();
}

class _ReceiveCoinPageState extends State<ReceiveCoinPage> {
  ...
  Future<void> _loadWallets(String evmAddress) async {
    setState(() {
      wallets = [
        {
          "name": "ETH",
  ...
  void _selectWallet(Map<String, dynamic> wallet) {
    if (wallet['name'] == selectedWallet) {
      ...
      Get.toNamed('/ReceivePage', arguments: {
        "currentNetwork": Get.arguments["currentNetwork"],
